...
from .datasets import CIFAR100_truncated, CIFAR100InstanceSample
...
def _moon_data_transforms_cifar100():
...
def get_dataloader_CIFAR100(datadir, train_bs, test_bs, dataidxs=None):
    ...
    # transform_train, transform_test = _moon_data_transforms_cifar100()
...
def pub_random_loaders_cifar100(datadir, args=None):
    """
    Borrowed code from https://github.com/WenkeHuang/FCCL/blob/main/fccl%2B/datasets/utils/public_dataset.py
    This function is used to create a dataloader from the public dataset for the study of FedFD.
    Args:
        datadir:
        args: global arguments from the main.

    Returns: public dataloader with random subsamples, sample number is equal to args.N_alignment

    """
    dl_obj = CIFAR100_truncated
    transform_train, transform_test = _data_transforms_cifar100()
    public_scale = args.N_alignment
    ...
    batch_size = args.pub_bs

    train_dl = data.DataLoader(train_ds, batch_size=batch_size, sampler=train_sampler, drop_last=True)

    return train_dl


def pub_random_loaders_cifar100_crd(datadir, args=None):
    dl_obj = CIFAR100InstanceSample

    transform_train, transform_test = _data_transforms_cifar100()
    _, _, _, _, net_dataidx_map, _ = \
        partition_data(dataset=None, datadir=datadir, partition='hetero',
                       n_nets=int(50000/args.N_alignment), alpha=1000)

    crd_args = {'k': args.nce_k, 'mode': args.nce_mode, 'is_sample': True, 'percent': 1.0, 'dataidxs': net_dataidx_map[0]}
    train_ds = dl_obj(root=datadir, download=True, train=True, transform=transform_train, **crd_args)

    n_train = len(train_ds)
    train_dl = data.DataLoader(train_ds, batch_size=args.pub_bs, shuffle=True, drop_last=True)

    return train_dl, n_train
